	...
	for "_c" from 0 to (count _cfgFunctions - 1) do {
		...
		if (isclass _currentTag) then {
			...
			if (_requiredAddonsMet) then {
				...
				//--- Initialize tag
				private ["_tag","_tagName","_itemPathTag","_itemRecompileTag"];
				...
				_itemRecompileTag = getnumber (_currentTag >> "recompile");
				...
				for "_i" from 0 to (count _currentTag - 1) do {
					...
					if (isclass _currentCategory) then {
						private ["_categoryName","_itemPathCat","_itemRecompileCat"];
						_categoryName = configname _currentCategory;
						_itemPathCat = gettext (_currentCategory >> "file");
						// _itemRecompileCat = getnumber (_currentCategory >> "recompile");
						_itemRecompileCat = 0;
						...
						for "_n" from 0 to (count _currentCategory - 1) do {
							...
							if (isclass _currentItem) then {
								...
								_itemMeta = [_itemPath,_itemExt,_itemHeader,_itemPreInit > 0,_itemPostInit > 0,(_itemRecompileTag > 0 || _itemRecompileCat > 0 || _itemRecompile > 0),_tag,_categoryName];
								...
								if (typename _itemCompile == typename {}) then {
									...
									if (_itemRecompileTag > 0 || _itemRecompileCat > 0 || _itemRecompile > 0) then {
										if (_pathAccess == 0) then {

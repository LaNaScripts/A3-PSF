...
_ctrlHealth = _display displayCtrl 5001; //2
_ctrlDir = _display displayCtrl 5003;  //3
_ctrlFps = _display displayCtrl 5004; //4
_ctrlRating = _display displayCtrl 5005; //1
  
while {!isNull _display} do {
	_ctrlFps ctrlSetText format["%1",round diag_fps];
	_ctrlHealth ctrlSetText format["%1%2", round((1 - (damage player)) * 100), "%"];
	_ctrlRating ctrlSetText format["%1",rating player];
	...
	if ("ItemCompass" in ([] call gA3PSF_fnc_allGear)) then { //Not worried about were the item is long as they have one.
